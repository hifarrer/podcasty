...
Return JSON ONLY in the following shape (ensure turns are labeled strictly with A or B, not names).${opts.generateVideo ? ` Additionally, split the spoken content into exactly ${Math.max(1, Math.floor((Math.min(3, opts.targetMinutes || 1)) * 2))} parts (one part per 30 seconds of target duration), returning a key-indexed object where keys are 1..N and values are phrases that take approximately 30 seconds each at the estimated_wpm. Each part should contain approximately ${Math.floor(150 * 0.5)} words (30 seconds at 150 WPM) to ensure proper duration. For example, a 1-minute target should have 2 parts, a 2-minute target should have 4 parts, etc. The split should cover the whole narration flow from intro to outro. Name this field parts30s.` : ''}
{
  "title": "...",
  "ssml": "<speak>...</speak>",
