...
Return JSON ONLY in the following shape (ensure turns are labeled strictly with A or B, not names). Additionally, split the spoken content into ~20-second phrases based on the estimated_wpm and targetMinutes (or total content), returning a key-indexed object where keys are 1..N and values are short phrases that take ~20 seconds each to read at the estimated_wpm. The split should cover the whole narration flow from intro to outro. Name this field parts20s:
{
  "title": "...",
  "ssml": "<speak>...</speak>",
